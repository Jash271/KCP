...
def fetch_user_token():
    try:
        ...
        response = requests.request(
            "POST", user_micro_svc_url, headers=headers, data=payload)
...
def get_email_data(service, message_id):
    msg = service.users().messages().get(
        id=message_id, userId='me', format='raw').execute()
...
def fetch_mails(file_pth, last_timestamp):
    ...
    try:
        ...
        res = service.users().messages().list(userId='me', maxResults=50,
                                              q=f'after:{last_timestamp}', labelIds='INBOX').execute()
        ...
        for msg in res['messages']:
            sender, subject, body, dateTime = get_email_data(
                service, msg['id'])
            body = cleanBody(body)
        ...
        df = pd.DataFrame(
            list(zip(msgIdList, senderList, subjectList, bodyList, dateTimeList)))
        print(df.head()[3])
        ...
        last_date = datetime.datetime(int(last_date_year), int(last_date_month), int(
            last_date_day), int(last_date_hr), int(last_date_min), int(last_date_sec))
...
def cleanBody(body):
    ...
    body = re.sub('[%s]' % re.escape(
        """!"#$%&'()*+,-/;<=>?@[\]^_`{|}~"""), ' ', body)  # remove punctuations
